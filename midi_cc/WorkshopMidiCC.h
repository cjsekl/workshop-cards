...
//   Card ID    -> CC 44-48  (5 x 7-bit = 35 bits, sent directly)
static constexpr uint8_t WS_CC_MAP[WS_NUM_CHANNELS] = {
    39, 38, 40, 41, 34, 35, 36, 37, 42, 43
};
...
public:
    ...
    int update(const uint16_t values[WS_NUM_CHANNELS]) {
        ...
        for (int i = 0; i < WS_NUM_CHANNELS; i++) {
            ...
            int8_t diff = val7 - _lastCC[i];
            if (diff < 0) diff = -diff;
            // Hysteresis: require change of 4+ to filter ADC noise on floating inputs
            if (_lastCC[i] < 0 || diff >= 4 || val7 == 0 || val7 == 127) {
                if (val7 != _lastCC[i]) {
